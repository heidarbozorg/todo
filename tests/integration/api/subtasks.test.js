...
describe('/api/subtask', () => {
    ...
    describe('POST /', () => {
        ...
        it('when passing wrong title should return "Bad request"', async () => {
            const res = await POST({ title: '', todo_id: 1 });
            expect(res.status).toBe(400);
        });


        it('When passing wrong todo_id should return "Bad request"', async () => {
            const res = await POST({ title: 'A title' });
            expect(res.status).toBe(400);
        });

        it('When post a not exists parent record, should return "Not found"', async () => {
            const apiResult = await POST({ title: 'A title', todo_id: 1 });
            expect(apiResult.status).toBe(404);
        });

        it('When post valid record, should return "Created"', async () => {
        ...
        it('when post valid record, should affect database', async () => {
    ...
    describe('PUT /', () => {
        ...
        it('When passing invalid id should return "Bad request"', async () => {
            const res = await PUT({ status: 1 });
            expect(res.status).toBe(400);
        });

        it('When passing invalid status should return "Bad request"', async () => {
            const res = await PUT({ status: 2 }, `/api/subtask/1`);
            expect(res.status).toBe(400);
        });

        it('When passing not exist id return "Not found"', async () => {
            const res = await PUT({ status: 1 }, `/api/subtask/1`);
            expect(res.status).toBe(404);
        });

        it('When put status=1 should affect at database', async () => {
            let sqlCommand = `INSERT INTO public."Todo" (title) VALUES ('integration test') RETURNING id;`;
